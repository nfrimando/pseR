...
params:
  stock_type:
    label: "Stock Type"
    input: select
    choices: [Blue Chip, Alternative, Concept Play, My Portfolio]
    value: "Blue Chip"
---
...
This report outlines `r params$stock_type` companies with their support and resistance bands scraped from [Investagram](https://www.investagrams.com/Home/).
...
stock_code_info.dt <- 
  ...
  rbind(
    data.frame(
      codes = c("AXLM", "FRUIT", "DNL", "WLCON", "HOME", "EEI", "MAC", "MWIDE",
                "ACEPH", "FB", "SSI", "BLOOM", "MPI"),
      class = rep("Alternative", 13)
    )
  ) %>% 
  # Concept Play
  rbind(
  ...
  rbind(
    data.frame(
      codes = c("FMETF", "ABS", "ALI", "BPI", "FGEN", "FRUIT", "LTG", 
                "MAXS", "MBT", "MER", "PIZZA"),
      class = rep("My Portfolio", 11)
    )
  )

top_codes <- (stock_code_info.dt %>% filter(class == params$stock_type))$codes %>% length()

stocks.dt <- scrape_investagram(as.character((stock_code_info.dt %>% filter(class == params$stock_type))$codes)) %>% 
  map_df(
    function(x) {
...
summary.dt <- stocks.dt %>% 
  ...
  select(code, close, support_1, resist_1, 
...
summary.dt %>%
  ...
  select(code, close, support_1, support_2, perc_diff_support_1, perc_diff_support_2,
...
summary.dt %>%
  ...
  select(code, close, resist_1, resist_2, perc_diff_resist_1, perc_diff_resist_2,
